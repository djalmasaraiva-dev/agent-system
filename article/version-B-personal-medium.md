...
For that shape of problem, the partner IGA I advocate is **SailPoint Agent Identity Security**, built on the **SailPoint Atlas** platform and integrated with **SailPoint Identity Security Cloud (ISC)** and **Data Access Security**. The reasons are also the architectural pattern, so it is worth being specific.

**Connector breadth across the major agent surfaces.** SailPoint Agent Identity Security connects directly to AWS, Azure, Google Cloud Platform, Salesforce, and Microsoft Copilot Studio — the surfaces where enterprises actually deploy agents — with extensibility to any other platform via SailPoint's Web Services connector pattern. That is exactly where a portable `/.well-known/agent-identity` endpoint plugs in for self-hosted ADK deployments behind the IGA.
...
**Separation-of-duties patterns extended to agents.** This is the one most engineering teams haven't yet named. The same separation-of-duties pattern SailPoint applies to human identities — detecting a single principal that holds conflicting privileges across the access map — extends to agents. An early coordinator agent that, via transitively reachable tool permissions, can both initiate and approve the same write path is a toxic combination by another name. Most teams building agents do not have language for this class of risk yet. The IGA discipline does.
...
For the architectural shape this article describes, that work lands cleanly on **SailPoint Agent Identity Security**, built on the SailPoint Atlas platform and anchored in the same Identity Security Cloud that already governs employees in most regulated FS shops. Other Google Cloud customers will reasonably choose differently. The architectural shape is what matters: native runtime identity inside each platform, security posture visibility across platforms, and a partner IGA layer above all of them, anchored in the HR system that already governs human identities.
...
*Diagram credits: Figure 4 follows the Google Cloud visual style. Figures 1–3 and 5–8 are editorial illustrations by the authors used to convey concept-level architecture, not official Google product or SailPoint diagrams.*
