...
When I scope an IGA layer for agents in a regulated Financial Services estate, the requirements are concrete: aggregate agents from every platform in scope, sync agent ownership to the HR-driven joiner-mover-leaver pipeline, run quarterly certification campaigns auditors will actually accept, surface SoD violations across agent → tool → data paths, and produce a single governed view that crosses clouds.

For that shape, in FS shops already running SailPoint for human identity — most of the regulated tier in Brazil — extending the same control plane to agents is the natural move. The product is [**SailPoint Agent Identity Security**](https://www.sailpoint.com/products/agent-identity-security), on the **SailPoint Atlas** platform, integrated with [**Identity Security Cloud (ISC)**](https://www.sailpoint.com/identity-security-cloud) and **Data Access Security (DAS)**. The ISC modules map cleanly to the requirements above: **Lifecycle Management** for JML and ownership succession, **Compliance Management** for certifications and SoD policies, **Access Modeling** for role-aware permission analysis. DAS extends the same framework to the data layer — the BigQuery and Snowflake stores the agents' `data_scopes` actually point at.

**Connector breadth across the major agent surfaces.** Agent Identity Security shipped with purpose-built connectors for **AWS, Entra (Microsoft), and Google agents** in the initial release, with SaaS variants for **Salesforce, ServiceNow, Snowflake**, plus a **Generic REST / Web Services** connector for the long tail ([SailPoint Developer Community](https://developer.sailpoint.com/discuss/t/onboarding-ai-agents-with-sailpoint-connectors/196492)). For Vertex AI / Gemini Enterprise Agent Platform, the Google agents connector is the native path; Web Services is where a portable `/.well-known/agent-identity` endpoint plugs in for self-hosted ADK on Cloud Run or GKE. The full [connectivity catalog](https://www.sailpoint.com/products/connectivity-and-integrations/catalog) covers the rest. ISC is also available on [**AWS Marketplace**](https://aws.amazon.com/marketplace/pp/prodview-pz66rdhrnioru), with broader cloud-marketplace listings expanding.

**Where the public picture has gaps.** SailPoint frames the stack as turnkey — "install the connector, authenticate, agents flow in." That fits the SaaS surfaces. For Vertex AI it's less specific: public materials don't say whether the Google connector covers Agent Engine managed runtime, ADK self-hosted on Cloud Run, GKE, or which attributes flow in beyond the agent principal itself. For ADK on Cloud Run inside a VPC-SC perimeter, the realistic path is mixed — native connector where it fits, Web Services with a portable `/.well-known/agent-identity` endpoint where it doesn't, with documented 429 rate-limit gotchas outside a Virtual Appliance. The marketing reads cleaner than the engineering. None of this is a regression — it's the same connector strategy you'd use for any non-trivial last-mile system — but worth saying out loud.
...
**The honest comparison.** [Saviynt](https://saviynt.com/) solves the same problem with the same shape, and is the natural pick where it already runs the human stack. [Okta for AI Agents](https://www.okta.com/products/govern-ai-agent-identity/) (GA April 2026) is stronger on runtime authorization primitives (Auth0 FGA at query level, short-lived credentials by default) and lighter on certification and SoD depth — a better fit when the heavier need is fine-grained access enforcement rather than full IGA lifecycle. SailPoint wins on footprint: in the FS shops I work with, ISC is already the lifecycle engine for humans, and Atlas plus DAS plus CIEM extend cleanly to the data and entitlement surfaces these agents actually touch.
...
- SoD policy violations applied across agents and the tools they reach
...
*Disclosure: no commercial or advisory relationship with SailPoint, Saviynt, or Okta. Products evaluated on customer engagements; advocated where the architectural fit is strong.*
